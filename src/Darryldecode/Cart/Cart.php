<?php namespace DevRooy\Cart;

use DevRooy\Cart\Exceptions\InvalidConditionException;
use DevRooy\Cart\Exceptions\InvalidItemException;
use DevRooy\Cart\Helpers\Helpers;
use DevRooy\Cart\Validators\CartItemValidator;
use DevRooy\Cart\Exceptions\UnknownModelException;

/**
 * Class Cart
 * @package DevRooy\Cart
 */
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = "\\DevRooy\\Cart\\CartCondition";

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of DevRooy\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) throw new InvalidConditionException('Argument 1 must be an instance of \'DevRooy\Cart\CartCondition\'');
